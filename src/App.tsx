...
// Memoize the authenticated app component
const AuthenticatedApp = React.memo(({ triggerCongrats }: { triggerCongrats?: (msg?: string) => void }) => {
  const { user } = useUser();
...
// Memoize the login/signup screen component
const LoginSignupScreen = React.memo(() => {
  ...
  }, [mode, login, signup, username, pin, navigate]);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background">
      <form onSubmit={handleSubmit} className="bg-white/80 dark:bg-background rounded-xl p-6 shadow-lg flex flex-col gap-4 w-80 max-w-full">
        ...
        <button type="submit" className="bg-gradient-to-r from-blue-500 to-green-400 text-white rounded py-2 font-semibold text-lg mt-2">
...
const App = () => {
  ...
  const triggerCongrats = React.useCallback((msg?: string) => {
    setCongratsMsg(msg);
    setShowCongrats(true);
  }, []);

  const handleCongratsClose = React.useCallback(() => {
